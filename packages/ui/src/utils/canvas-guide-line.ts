...
const Keys: <T = Record<string, any>>(obj: T) => (keyof T)[] = (obj) => {
...
export class AlignGuidelines {
  ...
  private getObjMaxWidthHeightByCoords(coords: ACoordsAppendCenter) {
    const objHeight =
      Math.max(Math.abs(coords.c.y - coords['tl'].y), Math.abs(coords.c.y - coords['tr'].y)) * 2;
    const objWidth =
      Math.max(Math.abs(coords.c.x - coords['tl'].x), Math.abs(coords.c.x - coords['tr'].x)) * 2;
