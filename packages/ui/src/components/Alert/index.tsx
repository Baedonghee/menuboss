...
import CheckFilled from '../SVG/icons/check-filled';
import CloseRound from '../SVG/icons/close-round';
  ...
  return (
    <AlertWrapper>
      <Box display="flex" justifyContent="space-between" mb="24px">
        {(type === 'check' || type === 'success') && (
          <CheckFilled width="32" height="32" color={theme.color.green500} />
        )}
        {type === 'error' && <Warning width="32" height="32" color={theme.color.red500} />}
        {(type === 'warning' || type === 'info') && (
          <Warning width="32" height="32" color={theme.color.primary500} />
        )}
        <CloseRound
          className="close"
          width="24"
      ...
      <Typography fontSize={theme.fontSize.text16} mb="32px">
        {parse(description)}
      </Typography>
      <Box display="flex" justifyContent="end">
        ...
        <Button
          ...
          onClick={onConfirm}
          color={
            type === 'check' || type === 'warning' || type === 'success' || type === 'info'
              ? 'primary'
              : 'error'
          }
        >
