...
const AddPlaylist: React.FC<IAddPlaylist> = ({ selectPlaylist, onClose, handleSelectPlaylist }) => {
  ...
  return (
    <ModalLayout title="재생목록 추가" onClose={onClose} width="1000px">
      <AddPlaylistWrapper mt="32px">
        ...
        {(loading || !!list.length) && (
          <ul className="list">
            ...
            {loading
              ...
              : list.map((item) => (
                  <li key={`playlist-${item.playlistId}`} onClick={() => handleCheck(item)}>
                    <div>
                    ...
                    <div>
                      ...
                      <Typography fontSize={theme.fontSize.text14} color={theme.color.gray900}>
                        {`콘텐츠${item.property.contentTypes.length}개`}
                      </Typography>
                    </div>
        ...
        {!loading && !list.length && (
          <Empty
            ...
            text="현재 저장된 재생목록이 없습니다<br/>재생목록을 추가하여 만들어주세요"
            minHeight="214px"
            size="s"
          />
        )}
