    ...
    <li className={classNames({ active })}>
      ...
      <Box mt="20px">
        {checked && (
          ...
          >
            {formatter.addComma(String(Number(formatter.onlyNumber(item.monthPrice)) * 12))} 원
          </Box>
        )}
        <Box display="flex" alignItems="center" mt={checked ? '4px' : '0px'}>
          <Typography
            fontSize={theme.fontSize.text20}
          ...
          >
            {checked
              ? item.name !== 'Enterprise'
                ? `${formatter.addComma(String(formatter.onlyNumber(item.yearPrice)))}원`
                : ''
              : item.name !== 'Enterprise'
                ? `${formatter.addComma(String(Number(formatter.onlyNumber(item.monthPrice))))}원`
                : ''}
          </Typography>
          {item.perScreen && (
            <Typography
              ...
              ml="12px"
            >
              {checked ? '/ 연 (TV 1개 기준)' : item.perScreen}
            </Typography>
          )}
        </Box>
      </Box>
