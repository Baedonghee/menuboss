...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    switch (method) {
      case 'POST': {
        const { data }: AxiosResponse<IApi> = await axios.post(
          paymentApi.createPaymentMethod.replace(':id', query.id as string)
        );
        if (data.status === 200) {
          res.json(data);
          return;
      ...
      case 'DELETE': {
        const { data }: AxiosResponse<IApi> = await axios.delete(
          paymentApi.deletePaymentMethod.replace(':id', query.id as string)
        );
        if (data.status === 200) {
          res.json(data);
          return;
