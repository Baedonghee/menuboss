...
const ScreenApply: React.FC<IScreenApply> = ({ type }) => {
  ...
  const handleScreenApply = async () => {
    try {
      handleShowAlert({
        title: 'Apply to screen?',
        description: 'Are you sure you want to apply it directly to the screen?',
        alertType: 'confirm',
        type: 'warning',
        confirmText: 'Apply',
        onConfirm: async () => {
          try {
            ...
            handleClose();
            successToast('Successfully applied to screen');
      ...
      <ScreenApplyWrapper mt="24px">
        ...
        {!loading && !list.length && (
          <Empty
            ...
            text={
              type === 'Schedule'
                ? 'There are currently no saved schedule<br/>Please register or create a schedule'
                : 'There are currently no saved playlists<br/>Please register or create a playlist'
            }
          />
        )}
      </ScreenApplyWrapper>
