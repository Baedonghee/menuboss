  ...
  const handlePlaylistPreview = async () => {
    try {
      if (playlistsErrorFile.length) {
        errorToast('Please check the duration of the file.');
        return;
      }
      let error = false;
      list.forEach((item) => {
    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          <Box mt="24px" display="flex" justifyContent="end" p="0px 20px">
            <Typography
            ...
            <ul className="file-list" ref={scrollRef}>
              {list.length ? (
                ...
                <Empty
                  ...
                  text="Playlist is empty<br/>Please select a content and add it"
                  minHeight="591px"
                />
              )}
            </ul>
