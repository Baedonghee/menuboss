...
import { IPlayList, IScheduleForm, IScheduleSettingList } from '@repo/ui/types';
...
const ScheduleSettings = () => {
  ...
  const [basicSchedule, setBasicSchedule] = useState<IScheduleSettingList | null>(null);
  ...
  const [scheduleList, setScheduleList] = useState<IScheduleSettingList[]>(
  ...
  const handleDelete = (index: number) => {
    if (scheduleList[index].playlist) {
      handleShowAlert({
        ...
        description: 'Are you sure you want to remove this playlist from your account?',
        alertType: 'confirm',
        confirmText: 'Delete',
        type: 'error',
        onConfirm: () => {
  ...
  const overlappingCheck = (checkLIst: IScheduleSettingList[]) => {
  ...
  const handleScheduleSave = async () => {
    ...
    if (errorIndexList.length) {
      errorToast('Please check the time settings');
      return;
    }
