    ...
    switch (method) {
      case 'GET': {
        const { data }: AxiosResponse<IApi> = await axios.get(
          scheduleApi.getSchedule.replace(':id', query.id as string)
        );
        if (data.status === 200) {
          res.json(data);
          return;
      ...
      case 'PATCH': {
        const { data }: AxiosResponse<IApi> = await axios.patch(
          scheduleApi.updateSchedule.replace(':id', query.id as string),
          body
        );
        if (data.status === 200) {
          res.json(data);
      ...
      case 'DELETE': {
        const { data }: AxiosResponse<IApi> = await axios.delete(
          scheduleApi.deleteSchedule.replace(':id', query.id as string)
        );
        if (data.status === 200) {
          res.json(data);
          return;
